...
const DEFAULT_CONFIG_BASE = require$$0.join(process.cwd(), cf);

console.log("DEFAULT_CONFIG_BASE======", DEFAULT_CONFIG_BASE);
...
const yargsParser = function Parser(args, opts) {
  console.log("args.slice()=", args.slice());
  console.log("opts=", opts);
  // 截取参数
  const result = parser.parse(args.slice(), opts);
  console.log("result.argv===", result.argv);
  return result.argv;
};
...
function printTimings(timings) {
  Object.entries(timings).forEach(([label, [time, memory, total]]) => {
    const appliedColor =
      label[0] === "#"
        ? label[1] !== "#"
          ? loadConfigFile_js.underline
          : loadConfigFile_js.bold
        : (text) => text;
    const row = `${label}: ${time.toFixed(0)}ms, ${prettyBytes$1(
      memory
    )} / ${prettyBytes$1(total)}`;
...
async function build(inputOptions, warnings, silent = false) {
  ...
  if (!silent) {
    ...
    loadConfigFile_js.stderr(
      loadConfigFile_js.cyan(
        `\n${loadConfigFile_js.bold(inputFiles)} → ${loadConfigFile_js.bold(
          files.join(", ")
        )}...`
      )
    );
    ...
    for (const file of outputs) {
      ...
      if (outputs.length > 1)
        process$1.stdout.write(
          `\n${loadConfigFile_js.cyan(
            loadConfigFile_js.bold(`//→ ${file.fileName}:`)
          )}\n`
        );
  ...
  if (!silent) {
    warnings.flush();
    loadConfigFile_js.stderr(
      loadConfigFile_js.green(
        `created ${loadConfigFile_js.bold(
          files.join(", ")
        )} in ${loadConfigFile_js.bold(ms(Date.now() - start))}`
      )
    );
...
async function getConfigPath(commandConfig) {
  ...
  if (commandConfig.slice(0, 5) === "node:") {
    ...
    try {
      return require.resolve(`rollup-config-${pkgName}`, {
        paths: [process$1.cwd()],
      });
    } catch (_a) {
      try {
...
async function findConfigFileNameInCwd() {
  const filesInWorkingDir = new Set(
    await require$$0$1.promises.readdir(process$1.cwd())
  );
  ...
  if (inputSource && inputSource.length > 0) {
    if (inputSource.some((input) => input.includes("="))) {
      ...
      inputSource.forEach((input) => {
        ...
        const key =
          input.substring(0, equalsIndex) || rollup.getAliasName(input);
...
console.log("command.help==", command.help);
console.log("process$1.argv==", process$1.argv);
console.log("process$1.argv.length==", process$1.argv.length);
console.log("process$1.stdin.isTTY==", process$1.stdin.isTTY);
...
export { getConfigPath, loadConfigFromCommand, ms, printTimings };
